...
## Current inventory (17 notebooks)
...
| `MedNorm_S1_Mention_Full_Training.ipynb` | **`REAL_DATA_EXECUTED + ARTIFACTS_VERIFIED`** | Audit 0025. Real guarded Colab GPU full-training run for the S1 mention span+type head. Reuses the validated components rather than duplicating them: two-pass fingerprint bootstrap with one forced restart, NumPy `RandomState` + dummy AdamW ABI preflight, scoped S1 dependency health, governed corpus gate, production VnCoreNLP segmentation (asserted), tokenizer equivalence preflight, and slow-tokenizer character alignment. Training is authorized only by a **validated** smoke artifact and is pinned to the **immutable** model revision read from it; `main` is rejected. An explicit `CONFIRM_FULL_TRAINING` guard (default empty) raises `SystemExit` before model acquisition, the real optimizer, and any backward pass. Writes to a separate Drive directory (`s1_mention_full_training_v1`) with `latest`/`best` checkpoints, JSONL history, resolved config, validation metrics, and a full-training manifest; resume rejects the SMOKE_ONLY checkpoint. **EXECUTED on a real Colab GPU runtime**: 4 epochs, 2,976 optimizer steps, `validation_span_micro_f1 = 0.7194053623573136`, writing the complete artifact to `s1_mention_full_training_v1`. Audit 0031's read-only full-artifact validator was then run against that Drive artifact and **passed with zero failed conditions** (all six required files present, three-way checkpoint hash agreement, epoch/step accounting, pinned revision `f89e80b4...24b6c5`, config hash, checkpoint resume schema, and the smoke checkpoint confirmed NOT to be the initializer). The artifact records repository commit `36c520d`, a verified ancestor of HEAD. See Audit 0032 §7a. |
| `MedNorm_S1_Mention_InternalTest_Evaluation.ipynb` | **`REAL_DATA_EXECUTED`** (local mode) | Audit 0031/0032. Read-only, dual-mode: in Colab it uses the complete Drive artifact and Audit 0031's unweakened full-artifact gate; locally it uses the git-ignored `checkpoint/s1_mention_full_training_v1/best.pt` and the narrower best-checkpoint-only gate, which always reports `full_artifact_validated: false`. Forward-only (`model.eval()`, `torch.no_grad()`, `requires_grad_(False)`); no optimizer, no scheduler, no backward; never writes into the training artifact or `checkpoint/`. **Executed locally on CPU** over the governed `internal_test` split (1,045/1,045 aligned): held-out span micro F1 **0.746182**, token micro/macro F1 0.8454/0.8234; DIAGNOSIS 0.892, MEDICATION 0.885, SYMPTOM 0.694 F1; TEST_NAME and TEST_RESULT have no governed supervision. Neither this nor the validation figure is the organizer's complete metric. No organizer inference, no `output.zip`. |
| `MedNorm_E5_XLMR_MRC_NER_Training.ipynb` | `IMPLEMENTED_UNEXECUTED` | Audit 0036 upgrades the Audit 0035 scaffold into a Colab execution package: five versioned MRC queries plus query hash, query/context label masking, negative-query preflight, bounded smoke/full loops, six-file artifact custody, checkpoint save/reload validation, and the Phase-2 artifact validator. No local or recorded smoke/full training has executed. |
| `MedNorm_L4_Learned_Resolver_v2_Training.ipynb` | `IMPLEMENTED_UNEXECUTED` | Audit 0036 upgrades the Audit 0035 scaffold into a Colab execution package for frozen L3 proposal datasets and the compact learned-L4 MLP: target/loss preflight, bounded boundary action checks, smoke/full loops, six-file artifact custody, checkpoint save/reload validation, and the Phase-2 artifact validator. No local or recorded smoke/full training has executed. |
| `MedNorm_Phase2_Validation_Ablation.ipynb` | `IMPLEMENTED_UNEXECUTED` | Audit 0036. Post-training Colab notebook for read-only E5/L4 artifact validation, governed-validation-only ablation planning/evaluation, candidate profile manifest writing, and the future internal_test freeze gate. It has no internal_test execution evidence and must not be classified as validation-evaluated until real artifacts and a validation report hash exist. **Audit 0051** removed every E4 arm from the ablation plan: E4 is `RETIRED_FROM_ACTIVE_ARCHITECTURE`, so an arm requiring it could never become available. |
...
## Notebooks removed in Audit 0051

| Notebook | Reason |
| --- | --- |
| `MedNorm_E4_Clean_Training.ipynb` | E4 PhoBERT-W2NER is `RETIRED_FROM_ACTIVE_ARCHITECTURE`. Its only purpose was to train an expert that will never be trained; its Stage-3/Stage-4 paths were already permanently blocked, and its source package was deleted with it. Evidence: Audits 0043-0048. |
| `MedNorm_ZS0_Baseline_Submission.ipynb` | The pretrained-only ZS0 baseline is no longer the project direction. The notebook never completed a run, so it produced no measured result to preserve; its reusable parts were migrated into `llm/`, `mention_factory/offsets.py`, `linking/snapshot.py` and `specialists/assertion/cues.py`. Evidence: Audits 0048-0050. |

The row that named `MedNorm_E4_PhoBERT_W2NER_Training.ipynb` was stale: that filename was superseded by `MedNorm_E4_Clean_Training.ipynb` in Audit 0045 and deleted from git in commit `7a84c8c`, so the row described a path this repository had not tracked for several audits.

Audit 0051 found an **untracked** file at that path on disk (61,226 bytes, SHA-256
`80cf751e33ea98764715f1fc024882893ee3253868553699c8f08810a76fa227`) whose bytes
matched no commit in the object database, and which imported
`mednorm_vi.training.phase2.e4_runtime_io` — a module path removed in Audit 0045. The
owner decided `DELETE_OBSOLETE` and it was **deleted**; nothing replaced it, and no
filename-specific ignore rule was added. `tests/unit/test_notebooks.py` now rejects
every unexpected untracked notebook with no exceptions.
