...
## Current inventory (16 notebooks)
...
| `MedNorm_E4_PhoBERT_W2NER_Training.ipynb` | `IMPLEMENTED_UNEXECUTED` | Audit 0035. Deterministic Colab scaffold for E4 PhoBERT W2NER: Drive-only outputs, governed train/validation hash gate before model acquisition, W2NER grid/decoder preflight, smoke/full separation, incompatible smoke-resume guard, immutable manifest, and explicit full-training authorization. No local or recorded full training. |
| `MedNorm_E5_XLMR_MRC_NER_Training.ipynb` | `IMPLEMENTED_UNEXECUTED` | Audit 0035. Deterministic Colab scaffold for E5 XLM-R MRC-NER: five versioned type queries, query/context masking preflight, exact start/end pairing preflight, Drive-only outputs, immutable manifest, safe resume guard, and explicit full-training authorization. No local or recorded full training. |
| `MedNorm_L4_Learned_Resolver_v2_Training.ipynb` | `IMPLEMENTED_UNEXECUTED` | Audit 0035. Deterministic Colab scaffold for learned L4 v2: governed corpus hash gate, synthetic target/split preflight, grouped leakage check, DIAGNOSIS/SYMPTOM wrong-type target check, Drive-only outputs, immutable manifest, safe resume guard, and explicit full-training authorization. No local or recorded full training. |
