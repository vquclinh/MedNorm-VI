#!/usr/bin/env python3
"""Continued refinement of the E3 ViHealthBERT span/type head (Audit 0062).
...
CONTRACT_VERSION = "e3-boundary-refinement-0062-v1"
...
def batches(features: list[dict[str, Any]], size: int, *, pad_token_id: int, shuffle_rng):
    order = list(range(len(features)))
    if shuffle_rng is not None:
        shuffle_rng.shuffle(order)
    for start in range(0, len(order), size):
        chunk = [_expand(features[i]) for i in order[start : start + size]]
        yield pad_encoded_features(chunk, pad_token_id=pad_token_id)
...
def main() -> int:
    ...
    def masked_mention_loss(logits: Any, labels: Any, label_mask: Any) -> Any:
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in batches(
            train_features, per_device, pad_token_id=pad_token_id, shuffle_rng=rng
        ):
            ...
            with torch.autocast("cuda", dtype=amp_dtype, enabled=amp_dtype is not None):
                logits = model(ids, attention)
                loss = masked_mention_loss(logits, labels, label_mask)
