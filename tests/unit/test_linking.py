...
from mednorm_vi.kb.ontology import ONTOLOGY_ICD, ONTOLOGY_RXNORM  # noqa: E402
from mednorm_vi.kb.rxnorm.structured import Strength, StructuredDrug  # noqa: E402
from mednorm_vi.linking import pipeline as op  # noqa: E402
from mednorm_vi.linking import pruning as opr  # noqa: E402
from mednorm_vi.linking import reformulation as orf  # noqa: E402
from mednorm_vi.linking import reranking as orr  # noqa: E402
from mednorm_vi.linking import retrieval as ort  # noqa: E402
from mednorm_vi.linking import runtime as orun  # noqa: E402
from mednorm_vi.linking import sparse as osp  # noqa: E402
from mednorm_vi.models import registry as gm  # noqa: E402

CLI_PATH = ROOT / "scripts/run_mednorm.py"
...
def _graph_config() -> Any:
    from mednorm_vi.kb.index_cache import RuntimeConfig
...
def test_the_canonical_deployed_total_is_unchanged(tmp_path: Path) -> None:
    from mednorm_vi.models.acquisition import MANIFEST_NAME
...
def test_0082_adds_no_model_and_trains_nothing() -> None:
    sources = [
        *sorted((ROOT / "src/mednorm_vi/linking").glob("*.py")),
        CLI_PATH,
    ]
    for path in sources:
...
def test_the_dense_cache_contract_is_untouched_by_0082() -> None:
    ...
    from mednorm_vi.kb.semantic_cache import CACHE_MANIFEST_VERSION, CacheManifest
    ...
    text = (ROOT / "src/mednorm_vi/linking/runtime.py").read_text(encoding="utf-8")
...
def test_no_stub_markers_in_the_0082_production_path() -> None:
    ...
    for path in [*sorted((ROOT / "src/mednorm_vi/linking").glob("*.py")), CLI_PATH]:
...
def test_one_production_profile_declares_the_whole_system() -> None:
    import yaml

    config = yaml.safe_load(
        (ROOT / "configs/pipeline/production.yaml").read_text(encoding="utf-8")
    )
    for section in ("icd_index", "rxnorm_index", "max_new_tokens", "extraction",
                    "alias", "retrieval", "sparse", "confidence", "retrievers"):
        assert section in config, section
    assert config["extraction"]["alias_support_only"] is True
    assert config["retrievers"]["clinlinker_kb_gp"]["roles"] == ["diagnosis"]
    assert config["retrievers"]["biobert_mnli"]["enabled"] is False
    assert config["expected_deployed_parameters"] == CANONICAL_TOTAL
    assert 8 <= config["retrieval"]["candidate_cap"] <= 12


def test_there_is_exactly_one_production_cli_and_config() -> None:
    scripts = sorted(p.name for p in (ROOT / "scripts").glob("*.py"))
    assert scripts == ["run_mednorm.py"]
    profiles = sorted(p.name for p in (ROOT / "configs/pipeline").glob("*.yaml"))
    assert profiles == ["production.yaml"]


def test_the_production_cli_exposes_one_workflow() -> None:
    import importlib.util

    spec = importlib.util.spec_from_file_location("cli", ROOT / "scripts/run_mednorm.py")
    ...
    assert commands == ["acquire", "budget", "index", "package", "run"]
    assert cli.CANONICAL_PARAMETERS == CANONICAL_TOTAL


def test_the_pipeline_emits_one_output_not_a_variant_family() -> None:
    from mednorm_vi.production import pipeline as production

    assert production.ENTITY_POLICY and production.CANDIDATE_POLICY
    source = (ROOT / "src/mednorm_vi/production/pipeline.py").read_text(encoding="utf-8")
    for retired in ("tierA", "tierAB", "tierABC", "allnull"):
        assert retired not in source, retired
    assert "output_raw_dotless" in source


def test_no_production_module_references_a_retired_milestone() -> None:
    import ast
    import re as _re

    milestone = _re.compile(r"\b00(7\d|8\d)\b")
    for path in (ROOT / "src/mednorm_vi").rglob("*.py"):
        if "__pycache__" in str(path):
            continue
        source = path.read_text(encoding="utf-8")
        tree = ast.parse(source)
        # Docstrings may record which audit a rule came from - that is provenance, and the
        # git history is the only other place it lives. Executable code may not.
        docstrings = {
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Constant) and isinstance(node.value, str):
                parent_is_doc = any(
                    isinstance(p, ast.Expr) and p.value is node and id(p) in docstrings
                    for p in ast.walk(tree)
                )
                if parent_is_doc:
                    continue
                assert not milestone.search(node.value), f"{path}: {node.value[:60]}"
            elif isinstance(node, ast.Name):
                assert not milestone.search(node.id), f"{path}: {node.id}"
