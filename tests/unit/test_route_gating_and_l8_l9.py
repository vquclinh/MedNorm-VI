...
def _phase1b(path: Path):
    ...
    return graph, run_phase1b(graph, Phase1BConfig.load(
...
def test_required_evidence_is_enforced_not_merely_documented() -> None:
    ...
    spec = CaseSpec(
        case="CX", name="x", activated_specialists=(),
...
def test_diagnostics_report_all_five_required_views() -> None:
    result = run_document(
        FIX / "synthetic_medical_document.txt", _config(), mode="deterministic")
    assert result.route_gate is not None
    payload = result.route_gate.as_dict()
    for key in ("eligible_nodes_by_expert", "skipped_nodes_by_expert",
                "proposals_by_expert", "active_routes_by_node",
                "route_gate_reasons"):
...
def _candidate(code: str, score: float, channel: str) -> LinkedCandidate:
    return LinkedCandidate(code=code, score=score, channels=(channel,),
                           snapshot_id="snap")
...
def test_selection_is_not_a_fixed_slice() -> None:
    ...
    many, _ = _select_candidates(
        [_candidate(c, 0.9, "sparse") for c in "ABCDE"],
        safety_bound=CANDIDATE_SAFETY_BOUND)
...
def test_every_candidate_carries_a_retain_or_remove_reason() -> None:
    _codes, decisions = _select_candidates([
        _candidate("A", 0.9, "sparse"), _candidate("B", 0.2, "sparse"),
...
def test_real_pipeline_candidate_sizes_are_not_all_ten() -> None:
    result = run_document(
        FIX / "synthetic_medication_list.txt", _config(), mode="deterministic")
...
def test_decoding_is_deterministic_across_runs() -> None:
    first = run_document(FIX / "synthetic_medication_list.txt", _config(),
                         mode="deterministic")
    second = run_document(FIX / "synthetic_medication_list.txt", _config(),
                          mode="deterministic")
    assert to_submission_json(first.predictions) == to_submission_json(
        second.predictions)
...
def test_decode_entities_emits_only_what_the_cascade_accepted() -> None:
    ...
    assert decode_entities(
        (), (CascadeDecision("h1", False, 0.0, ("below_threshold",)),), (), ()) == ()
...
def _snapshots() -> LockedSnapshots:
    config = _config()
    return LockedSnapshots(icd10=load_index(config.icd_index),
                           rxnorm=load_index(config.rxnorm_index))


def _entity(organizer_type: str, candidates: list[str]) -> dict[str, object]:
    return {"text": "x", "type": organizer_type, "candidates": candidates,
            "assertions": [], "position": [0, 1]}
...
def test_a_code_outside_the_snapshot_is_rejected() -> None:
    outcome = validate_entity_candidates(
        _entity("CHẨN_ĐOÁN", ["NOT_A_REAL_CODE"]), _snapshots())
...
def test_a_duplicate_candidate_is_rejected() -> None:
    outcome = validate_entity_candidates(
        _entity("THUỐC", ["198440", "198440"]), _snapshots())
...
def test_candidates_on_a_type_without_an_ontology_are_rejected() -> None:
    ...
    outcome = validate_entity_candidates(
        _entity("TRIỆU_CHỨNG", ["J18.9"]), _snapshots())
    assert any(i.code == "kb.candidates_on_type_without_ontology"
...
def test_a_missing_snapshot_fails_rather_than_passing() -> None:
    outcome = validate_entity_candidates(
        _entity("CHẨN_ĐOÁN", ["A000"]), LockedSnapshots())
...
def test_a_code_not_offered_for_this_mention_is_rejected() -> None:
    ...
    outcome = validate_entity_candidates(
        _entity("CHẨN_ĐOÁN", ["A000"]), _snapshots(),
        offered_codes=["J18.9"])
...
def test_membership_validation_is_independent_of_the_retriever() -> None:
    ...
    for forbidden in ("linking.icd10", "linking.rxnorm", "linking.snapshot",
...
def test_l9_organizer_validation_still_passes_alongside_membership() -> None:
    result = run_document(FIX / "synthetic_laboratory.txt", _config(),
                          mode="deterministic")
    payload = json.loads(to_submission_json(result.predictions))
...
def test_packaging_stops_when_an_injected_code_is_outside_the_snapshot(
    ...
    from mednorm_vi.validator.kb_membership import KbMembershipViolation
    ...
    (input_dir / "1.txt").write_text(
        (FIX / "synthetic_medication_list.txt").read_text(encoding="utf-8"),
        encoding="utf-8")
    ...
    def _forging_linker(hypothesis, index):  # type: ignore[no-untyped-def]
        return LinkerResult(
            ...
            candidates=(LinkedCandidate(code=forged, score=0.99,
                                        channels=("injected",)),))

    monkeypatch.setattr(pipeline_module, "link_rxnorm", _forging_linker)

    with pytest.raises(KbMembershipViolation) as raised:
        run_input_dir(input_dir, output_zip=output_zip, config=config,
                      mode="deterministic")
...
def test_the_canonical_packaging_path_actually_invokes_the_membership_gate(
    ...
    that only calls it directly would not have noticed.
    """
    from mednorm_vi.inference import pipeline as pipeline_module

    calls: list[str] = []
    original = pipeline_module.validate_document_candidates

    def _recording(root, snapshots, **kwargs):  # type: ignore[no-untyped-def]
        calls.append(str(kwargs.get("document_id")))
        return original(root, snapshots, **kwargs)

    monkeypatch.setattr(pipeline_module, "validate_document_candidates", _recording)
    input_dir = tmp_path / "input"
    input_dir.mkdir()
    (input_dir / "1.txt").write_text(
        (FIX / "synthetic_medication_list.txt").read_text(encoding="utf-8"),
    ...
    run_input_dir(input_dir, output_zip=tmp_path / "output.zip",
