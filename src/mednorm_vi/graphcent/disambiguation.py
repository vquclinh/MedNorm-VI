...
RUBRIC = (
    ...
    "* Judge SEMANTIC EQUIVALENCE, not lexical overlap. Two phrases that share words may "
    "denote different concepts; two phrases with no words in common may denote the same "
    "one.\n"
    "* Never supply detail the text does not state. If a candidate adds a subtype, an "
    "anatomical site, a laterality, pregnancy, malignancy, acuity or chronicity that the "
    "mention does not state, it is a different concept - answer NULL.\n"
    "* For medications the same rule applies to dosage, strength, concentration, form and "
    "route: if the mention does not state it and the candidate does, that candidate is more "
    "specific than the text supports. If the ingredient differs at all, answer NULL.\n"
    "* You MAY choose a more specific child concept, but ONLY when the mention or its "
    "surrounding context actually states what makes it specific. Otherwise choose the "
    "concept at the level the text supports, or NULL.\n"
    "* If EVERY candidate would require an assumption the text does not support, answer "
    "NULL. That is the expected answer, not a failure.\n"
    "* Select several indices only when the mention genuinely requires several DISTINCT "
    "concepts. Two candidates that are the same concept at different levels of detail are "
    "alternative granularities, not multiple codes - choose one or answer NULL.\n"
