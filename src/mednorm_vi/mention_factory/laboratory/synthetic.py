...
# tracked deterministic default (`configs/resolution/boundary_type_resolver_v1.yaml`,
# `boundary.group_preference.test_result: value_only`) selects the value alone, so
# the gold below follows that policy and says so. It is a *policy*, not a confirmed
# fact. (This comment named `configs/resolution/resolver_v1.yaml` until Audit 0056a;
# that file was deleted with the retired Phase-1C-A resolver in Audit 0055.)
...
class SyntheticLabCase:
    ...
    def validate(self) -> None:
        ...
        for relation in self.relations:
            if not (
                0 <= relation.name_index < len(self.entities)
...
class _CaseBuilder:
    ...
    def label(self, fragment: str, entity_type: str, *, role: str = "") -> int:
        ...
        self.entities.append(
            SyntheticEntity(start, start + len(fragment), entity_type, fragment, role)
        )
...
def build_cases() -> tuple[SyntheticLabCase, ...]:
    ...
    cases.append(b.build("lab-colon-01", FAMILY_COLON, "colon with no whitespace, decimal point"))
    ...
    cases.append(
        b.build(
            "lab-decimal-comma-01", FAMILY_DECIMAL_COMMA, "decimal comma must not split the value"
        )
    )
    ...
    cases.append(
        b.build("lab-flag-01", FAMILY_FLAG, "flags are evidence, never part of the value span")
    )
    ...
    _row(b, "PLT", "250")
    cases.append(
        b.build("lab-missing-unit-01", FAMILY_MISSING_UNIT, "a unit-less result is still a result")
    )
    ...
    cases.append(
        b.build("lab-multiline-01", FAMILY_MULTILINE, "three rows under one section header")
    )
